...
def millermeeks_extract_issues(issues_html, base_link):
    issues_list_element = issues_html.find(class_='evo-content')
    issue_elements = issues_list_element.find_all(class_='evo-views-row')
    issues = []
    for issue_element in issue_elements:
        a_el = issue_element.find(class_='h3').find('a')
        issue = {
...
def millermeeks_extract_article_links(issue, base_link):
    print(issue)
...
def millermeeks_extract_article(article_link):
    try:
